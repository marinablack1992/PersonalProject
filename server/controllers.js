module.exports = {
    ...
    getUsersPropsReqs: (req, res, next) => {
        const db = req.app.get('db');
        db.get_users_props_reqs([req.user.id])
            .then(data => res.status(200).send(data))
            .catch(err => res.status(500).send(console.log('Get All Properties, Users, Requests Failed.', err)));
    },

    deleteRequest: (req, res, next) => {
        const db = req.app.get('db');
        const { id } = req.params
        db.delete_request([req.user.id, id])
            .then(data => res.status(200).send(data))
            .catch(err => res.status(500).send(console.log('Delete Request Failed', err)))
    }
}
